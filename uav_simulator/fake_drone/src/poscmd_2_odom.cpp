...
void px4OdomCallBack(const nav_msgs::Odometry msg)
{	
	rcv_odom = true;
	_odom_msg    = msg;
}
...
{	
	...
	if(rcv_odom)
	{
		...
		odom.pose = _odom_msg.pose;
		odom.twist = _odom_msg.twist;
	}
	else
	{
...
{        
    ...
    _px4_odom_sub  = nh.subscribe( "/mavros/local_position/odom", 1, px4OdomCallBack );
